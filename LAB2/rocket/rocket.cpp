...
int pX = 500, pY = 400;                          // позиция по X и Y
int  j = 70;
    ...
    case WM_PAINT:
        {
            ...
            HBITMAP bmw;
            HDC memdc;
            LPCWSTR path[4] = { L"rock6.bmp", L"rock5.bmp",L"rock4.bmp",L"rock1.bmp" }; // названия спрайтов
            memdc = CreateCompatibleDC(hdc);

            bmw = (HBITMAP)LoadImage(NULL, L"rocket.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
            SelectObject(memdc, bmw);
            BitBlt(hdc, 0, 0, 1920, 1080, memdc, 0, 0, SRCCOPY);            
            bmw = (HBITMAP)LoadImage(NULL, path[0], IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
            SelectObject(memdc, bmw);
            int i = 0;
            while (j)
            {
                ...
                SelectObject(memdc, bmw);
                BitBlt(hdc, pX, pY, 150, 300, memdc, 0, 0, SRCINVERT);
                Sleep(100);
                BitBlt(hdc, pX, pY, 150, 300, memdc, 0, 0, SRCINVERT);
                pY -= 10;
